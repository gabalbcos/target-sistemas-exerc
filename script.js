// DOM selection
const resposta1 = document.querySelector('.resultado-soma');
const resposta2 = document.querySelector('.resultado-fibo');
const resposta5 = document.querySelector('.resultado-string');
...
function somaTudo() {
  const somaValor = document.querySelector('#soma').value;
  let somaTotal = 0;
  for (let i = 0; i < somaValor; i += 1) {
    somaTotal += 1;
  }
  resposta1.textContent = somaTotal;
}
...
function checaFibonacci() {
  const num = document.querySelector('#numero-fibo').value;
  const check = calculaFibonacci(num);
  if (parseInt(check, 10) === parseInt(num, 10)) {
    resposta2.textContent = `${num} pertence à sequência Fibonacci`;
  }
  resposta2.textContent = `${num} não pertence à sequência Fibonacci`;
}
...
4 - Dois veículos (um carro e um caminhão) saem respectivamente de cidades opostas pela
mesma rodovia. O carro de Ribeirão Preto em direção a Franca, a uma velocidade constante
de 110 km/h e o caminhão de Franca em direção a Ribeirão Preto a uma velocidade
constante de 80 km/h. Quando eles se cruzarem na rodovia, qual estará mais próximo a
...
function inverteString() {
  const string = document.querySelector('#string-reverso').value;
  const splitString = string.split('');
  const reversedString = [];
  for (let i = string.length - 1; i > -1; i -= 1) {
    reversedString.push(splitString[i]);
  }
  resposta5.textContent = reversedString.join('');
}